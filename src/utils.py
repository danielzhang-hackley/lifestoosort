...
def screw_bolt_other(image):
    ...
    p1, p2, p3 = whole_box[0], whole_box[1], whole_box[2]
    l1 = mat_line(p1, p2)
    l2 = mat_line(p2, p3)
    concave_line = mat_line(notch1, notch2)
    ...
    approx = np.vstack([approx[: min(notch1_idx, notch2_idx) + 1],
                        approx[max(notch1_idx, notch2_idx): ]])
    # approx = approx[min(notch1_idx, notch2_idx): max(notch1_idx, notch2_idx) + 1]
...
if __name__ == "__main__":
    ...
    eq1 = mat_line((10, -1), (2, 4))
    eq2 = mat_line((0, 0), (1, -1))

    print(find_intersect(eq1, eq2))
